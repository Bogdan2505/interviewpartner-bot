...
- [x] Настроить H2 для разработки (dev)
- [x] Настроить PostgreSQL для продакшна (prod)
- [x] Подключить Flyway и первичную миграцию схемы
- [x] Тесты проходят с ddl-auto=validate (схема создаётся миграцией)
...
- [x] /help — справка
- [x] Полный flow 3.4: запрос партнёру + принять/отклонить + создание Interview
- [ ] Доделать 3.4: улучшить формат/длительность в запросе, хранение заявок не в памяти

## Этап 4: Система напоминаний

### Задача 4.1: Планировщик напоминаний
- [x] Включить @Scheduled задачи (EnableScheduling)
- [x] NotificationService: выбор интервью для проверки (ближайший час)
- [x] Планировщик запуска (ReminderScheduler) раз в минуту
- [x] Тесты: выбор интервью и вызов сервиса планировщиком

### Задача 4.2: Отправка напоминаний
- [x] Напоминание за 24 часа / 1 час / 15 минут (генерация Notification)
- [x] Дедупликация отправок (Notification.sentAt + unique(interview,type))
